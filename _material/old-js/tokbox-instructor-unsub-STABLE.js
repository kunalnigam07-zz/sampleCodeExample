...
var TBInstructor = {
    ...
    setCycleValues: function () {
        ...
        $('#spinner-1-members').val(streamLimit);
        $('#spinner-2-seconds').val(streamTimeLimit / 1000);
    },
    ...
    showHideStreams: function (deletedone, newone) {
        ...
        for (i = 0; i < TBInstructor.streamQueue.length; i++) {
            var qid = TBInstructor.streamQueue[i];
            if ((i + 1) <= streamLimit) {
                if ($('#subscribersDiv #stream_' + TBInstructor.streamQueue[i]).length > 0) {
                    // Don't show module - already shown
                    console.log('Already shown');
                } else {

                    //TBInstructor.streamEntered(TBInstructor.streams[TBInstructor.streamQueue[i]]);
                    //$('#subscribersDiv').append($('#stream_' + TBInstructor.streamQueue[i]).parent());
                    var streamid = TBInstructor.streamQueue[i];
                    var divid = 'stream_' + streamid;
                    var activeStreamsLength = Object.keys(TBInstructor.activeStreams).length;

                    OT_LayoutContainer.addStream(divid, false);

                    var stream = TBInstructor.streams[TBInstructor.streamQueue[i]];

                    console.log('Rotation Subscribe:');
                    console.log(stream);
                    ...
                    // Subscribe to stream
                    var subtoaudio = true;
                    if ($('#mute-all-button').hasClass('active')) {
                        subtoaudio = false;
                    }
                    var subscriber = TBInstructor.session.subscribe(stream, divid, {subscribeToAudio: subtoaudio, audioVolume: 50, fitMode: 'contain', height: '100%', width: '100%', style: {nameDisplayMode: 'on'}}, function (error) {
                        if (error) {
                ...
                // Only subscribe to audio if they're not muted
                //if (!$('#mute-all-button').hasClass('active')) {
                //    TBInstructor.subscribers[qid].subscribeToAudio(true);
                //}
                //TBInstructor.subscribers[qid].subscribeToVideo(true);
            } 
            if ((i + 1) > streamLimit) {
                if ($('#subscribersDiv #stream_' + TBInstructor.streamQueue[i]).length > 0) {
                    // It's live, trash it
                    //TBInstructor.streamExited(TBInstructor.streams[TBInstructor.streamQueue[i]]);
                    OT_LayoutContainer.removeStream('stream_' + TBInstructor.streamQueue[i]);
                    


                    TBInstructor.session.unsubscribe(TBInstructor.subscribers[TBInstructor.streamQueue[i]]);
                    if (TBInstructor.activeStreams[TBInstructor.streamQueue[i]]) {
                        delete TBInstructor.activeStreams[TBInstructor.streamQueue[i]];
                    }

                    console.log('Rotation UNSubscribe:');
                    console.log(TBInstructor.streams[TBInstructor.streamQueue[i]]);

                } 
                //TBInstructor.subscribers[qid].subscribeToAudio(false);
                //TBInstructor.subscribers[qid].subscribeToVideo(false);
            }
        }
